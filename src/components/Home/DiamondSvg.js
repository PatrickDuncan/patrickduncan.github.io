    ...
    return (
      <div className="table-cell">
        <svg
          ...
          className="no-select"
        >
          {Content.diamondSvg.points.map((diamond, i) => (
            <Polygon
              key={ i }
              points={ diamond }
              reAnimate={ this.reAnimate.bind(this) }
            />
          ))}
          <text
            className="hand"
            fill="#000"
            onClick={ this.reAnimate.bind(this) }
            style={ TEXT_STYLE }
            x="218"
            y="329"
          >
          ...
          <RotatingDiamonds reAnimate={ this.reAnimate.bind(this) } />
        </svg>
      </div>
    );
  }
